...
def main():
  info = subprocess.check_output(['/usr/sbin/megacli', '-AdpAllInfo', '-aAll', '-nolog']).decode('utf-8').splitlines()
  pdlist = subprocess.check_output(['/usr/sbin/megacli', '-PdList', '-aAll', '-nolog']).decode('utf-8').splitlines()
  out = {}
  adapter = None
